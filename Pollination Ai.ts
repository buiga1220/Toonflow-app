...
const vendor: VendorConfig = {
  ...
  name: "Pollinations (Miễn phí)",
  description: "## Pollinations.ai 免费绘画接口\n\n**Hoàn toàn miễn phí, không cần API Key!**\nHỗ trợ sinh ảnh chất lượng cao. Do là miễn phí nên tốc độ phản hồi có thể tuỳ thuộc vào tình trạng mạng.",
  inputs: [], // Không cần nhập key
  inputValues: {},
  models: [
    { 
...
const imageRequest = async (config: ImageConfig, model: ImageModel): Promise<string> => {
  ...
  // Thêm seed ngẫu nhiên để tránh trình duyệt/server cache lại ảnh cũ
  const seed = Math.floor(Math.random() * 1000000);
  const url = `https://image.pollinations.ai/prompt/${encodedPrompt}?width=${width}&height=${height}&nologo=true&seed=${seed}`;
  
  logger(`请求URL: ${url}`);
  
  // urlToBase64 sẽ tự động tải file từ URL về và biến thành chuỗi base64
